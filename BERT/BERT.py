...
# Libraries for BERT
from transformers import BertConfig, BertForSequenceClassification, BertTokenizer, get_cosine_schedule_with_warmup, get_linear_schedule_with_warmup
...
# Function that tokenizes the text in order to be input in BERTmodel
...
train_input_ids, train_attention_masks = tokenized_sentences(x_train.tolist(), max_len, bert_tokenizer)
val_input_ids, val_attention_masks = tokenized_sentences(x_val.tolist(), max_len, bert_tokenizer)
test_input_ids, test_attention_masks = tokenized_sentences(x_test.tolist(), max_len, bert_tokenizer)
...
# # Create a configuration with dropout = 0.2
# bert_config = BertConfig.from_pretrained(
#     "bert-base-uncased",
#     num_labels=2,
#     hidden_dropout_prob=0.2,      # Dropout for fully connected layers
...
#     config=bert_config
# )
# # Freeze the first 4 encoder layers of BERT to reduce overfitting
# for name, param in bert_model.bert.named_parameters():
#     if any(name.startswith(f"encoder.layer.{i}") for i in range(4)):
...
# Hyper-parameter tuning for BERT model
learning_rate = 2e-5
# learning_rate = 1e-5
# learning_rate = 3e-5
num_epochs = 2
...
# num_epochs = 2
clip_g = True
# optimizer = Adam(bert_model.parameters(), lr=learning_rate, betas=(0.9, 0.999),eps=1e-8)
optimizer = AdamW(bert_model.parameters(), lr=learning_rate, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.01)
...
# train model
bert_model, (tr_losses, val_losses, tr_accs, val_accs) = train_model(bert_model, train_dataloader, val_dataloader, device, optimizer, scheduler, num_epochs, learning_rate, clip_g, early_stop_patience=4)
...
with torch.no_grad():
    for batch in test_dataloader:
        ...
        y_test_pred += torch.argmax(probs, dim=1).cpu().tolist()

submission_ds = pd.DataFrame({"ID": test_ds["ID"], "Label": y_test_pred})
submission_ds.to_csv("submission.csv", index=False)

# Run BERT model for evaluation on validation dataset
bert_model.eval()
y_pred_val = []
val_probs = []
...
plt.title("Learning Curve for BERT Model")
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.legend()
plt.grid(True)
...
plt.title("Loss Curve for BERT Model")
plt.xlabel("Epochs")
plt.ylabel("Loss")
plt.legend()
plt.grid(True)
